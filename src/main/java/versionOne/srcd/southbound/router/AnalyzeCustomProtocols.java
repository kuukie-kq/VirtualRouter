...
public class AnalyzeCustomProtocols extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if(flagForward) {
            stringForward = stringForward + RouterUtils.FORWARD + "jiaoyan" + RouterUtils.SEQUENCE +
                    RouterUtils.SEQUENCE + RouterUtils.SEQUENCE + request;
            ctx.write(ctx.alloc().buffer().writeBytes(stringForward.getBytes(RouterUtils.charset)));
            ctx.fireChannelRead(stringForward);
        } else if(flagUpdate) {
            stringUpdate = stringUpdate + RouterUtils.UPDATE + "jiaoyan"  + RouterUtils.SEQUENCE +
                    RouterUtils.SEQUENCE + RouterUtils.SEQUENCE + request;
